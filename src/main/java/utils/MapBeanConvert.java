...
/**
 * Created by EalenXie on 2019/7/22 11:13.
 * 自定义 JavaBean 与 Map互相转换 工具类
 */
@SuppressWarnings("unchecked")
public class MapBeanConvert {

    private MapBeanConvert() {
    }
        ...
        for (PropertyDescriptor descriptor : propertyDescriptors) {
            ...
            if (isUnderlineToCamel) {
                if (map.containsKey(prop) || map.containsKey(camelToUnderline(prop))) {
                    ...
                    Object[] args = value == null ? null : new Object[]{value};
                    descriptor.getWriteMethod().invoke(result, args);
                }
            } else {
                if (map.containsKey(prop)) {
                    Object[] args = map.get(prop) == null ? null : new Object[]{map.get(prop)};
                    descriptor.getWriteMethod().invoke(result, args);
                }
            }
        }
